...
12. Verify that the sandbox instance was properly cleaned up:
  ...
  - Check that the instance isn't running in the cloud provider anymore and no storage volumes or containers are left.
...
**Test run**:
 ...
 8. Trigger sanity checks against the sandbox instance:
  ...
  - Confirm that the instance is accessible when passing the username and password during authentication.
  - Create a class, add a set of objects in batches to the sandbox instance.
  - Ensure that the stored objects can be queried.
  - Remove some of the added objects and ensure that they aren't present anymore in the instance.
  - Check that the displayed version from the instance details matches the service's version.
...
**Test preparation**:
 ...
 1. Create a WCS account to be able to login into the [WCS dashboard](https://console.weaviate.cloud/dashboard). Make sure that the login was successful
 2. Click in <kbd>+Create cluster</kbd> and make sure to select the <kbd>Free sandbox</kbd> tab.
 3. Add a cluster name with 32 characters, for example: *this-is-a-very-long-clusternames*.
...
**Test run**:
 ...
 1. Create a WCS account to be able to login into the [WCS dashboard](https://console.weaviate.cloud/dashboard). Make sure that the login was successful
 2. Click in <kbd>+Create cluster</kbd> and make sure to select the <kbd>Free sandbox</kbd> tab.
 3. Add a cluster name with 33 characters, for example: *this-is-a-very-loong-clusternames*.
...
**Test run**:
 ...
 1. Create a WCS account to be able to login into the [WCS dashboard](https://console.weaviate.cloud/dashboard). Make sure that the login was successful
 2. Click in <kbd>+Create cluster</kbd> and make sure to select the <kbd>Free sandbox</kbd> tab.
 3. Add a cluster name with special characters, for example: *españa*.
 4. Click on <kbd>Create</kbd>.
...
**Test run**:
 ...
 1. Create a WCS account to be able to login into the [WCS dashboard](https://console.weaviate.cloud/dashboard). Make sure that the login was successful
 2. Click in <kbd>+Create cluster</kbd> and make sure to select the <kbd>Free sandbox</kbd> tab.
 3. Add a cluster name with 1 character, for example: *w*.
 4. Click on <kbd>Create</kbd>.
...
**Test Description**: Attempt the creation, deletion and querying of data objects in a Sandbox instance configured with authentication by accessing the API endpoints directly without any authentication. Use an instance name with the minimum number of characters to verify that it's also allowed.

**Test preparation**:

 1. Login into the [WCS dashboard](https://console.weaviate.cloud/dashboard). Make sure that the login was successful.
 2. Click in <kbd>+Create cluster</kbd> and make sure to select the <kbd>Free sandbox</kbd> tab.
 3. Add a cluster name with two characters, for example: *w-*.
 4. Make sure that <kbd>Enable Authentication?</kbd> is set to YES.
 5. Click on <kbd>Create</kbd>.
 6. Wait for the WCS sandbox instance to be created.
...
**Test run**:
 ...
 9. Attempt the following scenarios by accessing directly the [API endpoints](https://weaviate.io/developers/weaviate/api/rest) without any authentication:
  - [List data objects](https://weaviate.io/developers/weaviate/api/rest/objects#list-data-objects) without authentication. Ensure that the request fails and no data is retrieved.
  - [Create data objects](https://weaviate.io/developers/weaviate/api/rest/-#create-a-data-object) without authentication. Ensure that the request fails and no data is stored in the Sandbox instance.
  - [Update data objects](https://weaviate.io/developers/weaviate/api/rest/-#update-a-data-object) without authentication. Ensure that the request fails and no data is retrieved.
