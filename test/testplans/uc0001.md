...
8. Trigger sanity checks against the sandbox instance:
  - Ensure that the cluster endpoint is reachable.
  - Ensure that no authentication is needed to access the WCS instance.
  - Create a class, add a set of objects in batches to the sandbox instance.
  - Ensure that the stored objects can be queried.
  - Remove some of the added objects and ensure that they aren't present anymore in the - ce.
  - Check that the displayed version from the instance details matches the service's - n.
  - Check that all the modules displayed in the instance details are really available in the instance.
...
**Test run**:
 ...
 8. Trigger sanity checks against the sandbox instance:
  - Ensure that the cluster endpoint is reachable.
  - Ensure that the instance is not accessible if the admin API key is not passed.
  - Verify that the instance is accessible when passing the admin API key.
  - Confirm that the instance is accessible when passing the username and password during - cation.
  - Create a class, add a set of objects in batches to the sandbox instance.
  - Ensure that the stored objects can be queried.
  - Remove some of the added objects and ensure that they aren't present anymore in the instance.
  - Check that the displayed version from the instance details matches the service's version.
  - Check that all the modules displayed in the instance details are really available in the instance.
...
**Test cleanup**:
 ...
 11. Wait for the WCS sandbox instance to be destroyed.
 12. Verify that the sandbox instance was properly cleaned up:
  - Make sure that the endpoint is not reachable.
  - Check that the instance isn't running in the cloud provider anymore and no storage volumes or - s are left.
  - Confirm that all associated resources are properly cleaned up, preventing unnecessary costs.
...
**Test cleanup**:
 ...
 12. Verify that the sandbox instance was properly cleaned up:
  - Make sure that the endpoint is not reachable.
  - Check that the instance isn't running in the cloud provider anymore and no storage volumes or -rs are left.
  - Confirm that all associated resources are properly cleaned up, preventing unnecessary costs.
...
**Test preparation**:
 ...
 1. Login into the [WCS dashboard](https://console.weaviate.cloud/dashboard). Make sure that the login was uccessful.
 2. Click in <kbd>+Create cluster</kbd> and make sure to select the <kbd>Free sandbox</kbd> tab.
 3. Add a cluster name, for example: test-sandbox.
 4. Make sure that <kbd>Enable Authentication?</kbd> is set to YES.
 5. Click on <kbd>Create</kbd>.
...
**Test run**:
 ...
 - Process 1: Authenticates using the API key and starts creating several schemas and updating them.
 - Process 2: Authenticates using the same API key and starts ingesting data.
 - Process 3: Authenticates using the same API key and performs several complex queries.
 - During the simulation time store: response time, throughput (API requests per second), concurrency level (number of simultaneous connections), sandbox instance resources (CPU, memory and disk usage).
 9. Analyze the retrieved data looking for performance bottlenecks.

**Test cleanup**:

 10. From the [WCS dashboard](https://console.weaviate.cloud/dashboard) allocate the created *test-sandbox* andbox instance and click on the rubbish bin for deletion.
...
**Test run**:
 ...
 9. Attempt the following scenarios by accessing directly the [API endpoints](https://weaviate.io/developers/weaviate/api/rest) without any authentication:
  - [List data objects](https://weaviate.io/developers/weaviate/api/rest/objects#list-data-objects) - authentication. Ensure that the request fails and no data is retrieved.
  - [Create data objects](https://weaviate.io/developers/weaviate/api/rest/-#create-a-data-object) without authentication. Ensure that the request fails and no data is stored -Sandbox instance.
  - [Update data objects](https://weaviate.io/developers/weaviate/api/rest/-#update-a-data-object) without authentication. Ensure that the request fails and no data is -ed.
  - [Delete data objects](https://weaviate.io/developers/weaviate/api/rest/objects#delete-a-data-object) without authentication. Ensure that the request fails and no data is retrieved.
...
**Test cleanup**:
 ...
 11. From the [WCS dashboard](https://console.weaviate.cloud/dashboard) allocate the created *test-sandbox* andbox instance and click on the rubbish bin for deletion.
...
**Test preparation**:
 ...
 1. Login into the [WCS dashboard](https://console.weaviate.cloud/dashboard). Make sure that the login was  ccessful.
 2. Click in <kbd>+Create cluster</kbd> and make sure to select the <kbd>Free sandbox</kbd> tab.
 3. Add a cluster name, for example: test-sandbox.
 4. Make sure that <kbd>Enable Authentication?</kbd> is set to YES.
 5. Click on <kbd>Create</kbd>.
...
11. From the [WCS dashboard](https://console.weaviate.cloud/dashboard) allocate the created *test-sandbox* ndbox instance and click on the rubbish bin for deletion.
12. Write the instance's name in the text box and click confirm.
13. Wait for the WCS sandbox instance to be destroyed.
14. Verify that the sandbox instance was properly cleaned up:
  - Make sure that the endpoint is not reachable.
